...
function startSimulation9(i){
	if(isConnected){
		state.bayID = i;
		console.log('parking9 is: '+ i);
		sendMQTTMessage9(i);
	}else{
		setTimeout(startSimulation9, 1000);
...
sendMQTTMessage9 = async(i) => {
	var origStatus = state.status;
	await fetchParking9(i);
	var newStatus = state.status;
	if (origStatus !== newStatus){	
		await fetchRestriction9(i)
		await device.publish('parking9', JSON.stringify(state));
	}
	timeout = setTimeout(sendMQTTMessage9, 1000);
}

fetchParking9 = async(id) => {
...
device
	.on('connect', () => {
		console.log('connected parking9');
...
module.exports = {
	state: state,
	startSimulation9: startSimulation9, 
